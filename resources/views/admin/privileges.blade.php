                            ...
                            <script type="text/javascript">
                                $(function () {
                                    ...
                                    $('#set_as_superadmin input').click(function () {
                                        if ($('#set_as_superadmin input:checked').val() === '1') {
                    ...
                    <div id='privileges_configuration' class='form-group'>
                            ...
                            <script>
                                $(function () {
                                    $("#is_visible").click(function () {
                                        $(".is_visible").prop("checked", $(this).prop('checked'));
                                    });
                                    $("#is_create").click(function () {
                                        $(".is_create").prop("checked", $(this).prop('checked'));
                                    });
                                    $("#is_read").click(function () {
                                        $(".is_read").prop("checked", $(this).is(':checked'));
                                    });
                                    $("#is_edit").click(function () {
                                        $(".is_edit").prop("checked", $(this).is(':checked'));
                                    });
                                    $("#is_delete").click(function () {
                                        $(".is_delete").prop("checked", $(this).is(':checked'));
                                    });
                                    $("#is_approve").click(function () {
                                        $(".is_approve").prop("checked", $(this).is(':checked'));
                                    });
                                    $(".select_horizontal").click(function () {
                                        $(this).parents('tr').find("input[type=checkbox]").prop("checked", $(this).is(':checked'));
                                    });
                                })
                            </script>
                        @endpush
                        <table class='table table-striped table-hover table-bordered'>
                            ...
                            <tr class='active'>
                                ...
                                <th width='45%'>{{trans('crudbooster.privileges_module_list_mod_names')}}</th>
                                <th>&nbsp;</th>
                            ...
                            <tr class='info'>
                                ...
                                <th>&nbsp;</th>
                                <th>&nbsp;</th>
                                <td align="center"><input title='Check all' type='checkbox' id='is_visible'/></td>
                                <td align="center"><input title='Check all' type='checkbox' id='is_create'/></td>
                                <td align="center"><input title='Check all' type='checkbox' id='is_read'/></td>
                                <td align="center"><input title='Check all' type='checkbox' id='is_edit'/></td>
                                <td align="center"><input title='Check all' type='checkbox' id='is_delete'/></td>
                                <td align="center"><input title='Check all' type='checkbox' id='is_approve'/></td>
                            </tr>
                            </thead>
                            <tbody>
                            <?php $no = 1;?>
                            @foreach($moduls as $modul)
                                ...
                                $roles = DB::table('cms_privileges_roles')
                                    ...
                                    ->first();
                                $supportsApprove = in_array($modul->path, $approve_module_paths, true);
                                ?>
                                <tr>
                                    <td><?php echo $no++;?></td>
                                    <td>{{$modul->name}}</td>
                                    <td class='info' align="center"><input type='checkbox' title='Check All' class='select_horizontal'/></td>
                                    ...
                                    <td class='privilege-approve' align="center">
                                        @if($supportsApprove)
                                            <input type='checkbox' class='is_approve' name='privileges[<?=$modul->id?>][is_approve]' <?=(@$roles->is_approve) ? "checked" : ""?> value='1'/>
                                        @else
