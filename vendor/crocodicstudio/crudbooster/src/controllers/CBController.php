...
class CBController extends Controller
{
    ...
    {
        if (! config('crudbooster.DATA_QUERY_ALLOWLIST_ENABLED', false)) {
            return response()->json([
                'message' => 'Data query endpoint is disabled',
            ], 403);
        }

        $query = Request::get('query');

        if (! $this->isAllowedDataQuery($query)) {
            return response()->json([
    ...
    private function isAllowedDataQuery($query)
    {
        if (! config('crudbooster.DATA_QUERY_ALLOWLIST_ENABLED', false)) {
            return false;
        }
        ...
        // Disallow multi statements and anything except read-only SELECT.
        ...
        if (preg_match('/\b(insert|update|delete|drop|truncate|alter|create|replace|rename|grant|revoke|call|exec(?:ute)?|set)\b/i', $query)) {
            return false;
        }

        $allowlist = config('crudbooster.DATA_QUERY_ALLOWLIST_TABLES', []);
        if (is_string($allowlist)) {
            $allowlist = explode(',', $allowlist);
        }

        $allowlist = array_values(array_filter(array_map(function ($table) {
        ...
        $tables = $this->extractTablesFromSelectQuery($query);
        if (! count($tables)) {
            return false;
        }

        foreach ($tables as $table) {
            if (! in_array($table, $allowlist)) {
                return false;
            }
        }
    ...
    private function extractTablesFromSelectQuery($query)
    {
        ...
        foreach ((array) $matches[1] as $table) {
            ...
            // Support schema.table notation by matching table name in allowlist.
    ...
    private function isSafeSqlIdentifier($identifier)
    {
    ...
    private function applySafeDatatableWhere($query, $datatableWhere, $table)
    {
        if (! is_string($datatableWhere)) {
            abort(400, 'Invalid datatable filter');
        }

        $datatableWhere = trim($datatableWhere);
        if ($datatableWhere === '') {
            return;
        }
        ...
        $clauses = preg_split('/\s+and\s+/i', $datatableWhere);
        foreach ((array) $clauses as $clause) {
            ...
            if (preg_match('/^([a-zA-Z_][a-zA-Z0-9_]*)\s+is\s+not\s+null$/i', $clause, $matches)) {
                $column = $matches[1];
                if (! Schema::hasColumn($table, $column)) {
                    abort(400, 'Invalid datatable filter column');
                }
                $query->whereNotNull($column);
                continue;
            }

            if (preg_match('/^([a-zA-Z_][a-zA-Z0-9_]*)\s+is\s+null$/i', $clause, $matches)) {
                $column = $matches[1];
                if (! Schema::hasColumn($table, $column)) {
                    abort(400, 'Invalid datatable filter column');
                }
            ...
            if (preg_match('/^([a-zA-Z_][a-zA-Z0-9_]*)\s+(not\s+like|like)\s+(.+)$/i', $clause, $matches)) {
                $column = $matches[1];
                $operator = stripos($matches[2], 'not') !== false ? 'not like' : 'like';
                $value = $this->parseDatatableWhereLiteral($matches[3]);
    ...
    private function parseDatatableWhereLiteral($literal)
    {
    ...
    public function getDataTable()
    {
        ...
        $datatableWhere = urldecode((string) Request::get('datatable_where'));
        $foreign_key_name = Request::get('fk_name');
        $foreign_key_value = Request::get('fk_value');
        if ($table && $label && $foreign_key_name && $foreign_key_value !== null && $foreign_key_value !== '') {
            if (! $this->isSafeSqlIdentifier($table)
                || ! $this->isSafeSqlIdentifier($label)
                || ! $this->isSafeSqlIdentifier($foreign_key_name)
                || ! Schema::hasTable($table)
            ) {
                abort(400, 'Invalid datatable request');
            }

            $query = DB::table($table);
            if ($datatableWhere !== '') {
                if (config('crudbooster.DATATABLE_WHERE_HARDENING_ENABLED', true)) {
                    $this->applySafeDatatableWhere($query, $datatableWhere, $table);
                } else {
    ...
    public function getUpdateSingle()
    {
        ...
        if (! config('crudbooster.UPDATE_SINGLE_HARDENING_ENABLED', true)) {
            return $this->getUpdateSingleLegacy();
        }

        if (! CRUDBooster::isUpdate() && $this->global_privilege == false) {
            CRUDBooster::insertLog(trans("crudbooster.log_try_update", [
                'name' => Request::get('id'),
        ...
        // Only the current module table may be updated (ignore/reject arbitrary table params).
        $table = $this->table;
        if ($requestedTable !== null && $requestedTable !== '' && $requestedTable !== $table) {
            abort(403, 'Table is not allowed');
        }

        $allowedColumns = $this->getUpdateSingleAllowedColumns();
        $passwordFields = array_map('trim', explode(',', config('crudbooster.PASSWORD_FIELDS_CANDIDATE')));
        $deniedColumns = array_merge($passwordFields, [
            $this->primary_key,
            'id_cms_privileges',
            'id_cms_moduls',
            'is_superadmin',
        ...
        if (! is_string($column)
            ...
            || in_array($column, $deniedColumns, true)
            || ! Schema::hasColumn($table, $column)
        ) {
            abort(403, 'Column is not allowed');
        }
    ...
    /**
     * Legacy CRUDBooster behavior — only used when CB_UPDATE_SINGLE_HARDENING_ENABLED=false.
     */
    private function getUpdateSingleLegacy()
    {
        $table = Request::get('table');
        $column = Request::get('column');
        $value = Request::get('value');
    ...
    /**
     * Columns that getUpdateSingle may write. Limited to form fields intended
     * for file clearing (upload / filemanager), matching the sole UI caller.
     */
    private function getUpdateSingleAllowedColumns()
    {
        $allowedTypes = ['upload', 'filemanager'];
        $columns = [];

        foreach ((array) $this->form as $form) {
            if (empty($form['name']) || empty($form['type'])) {
                continue;
            }
...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
    ...
    public function getImportData()
    {
        ...
        if (Request::get('file') && ! Request::get('import')) {
            $validatedFile = $this->validateImportFilePath(Request::get('file'));
            if (! $validatedFile['ok']) {
                abort($validatedFile['status'], $validatedFile['error']);
            }

            $rows = Excel::load($validatedFile['path'], function ($reader) {
            })->get();
    ...
    public function postDoImportChunk()
    {
        ...
        $validatedFile = $this->validateImportFilePath(Request::get('file'));
        if (! $validatedFile['ok']) {
            return response()->json(['status' => false, 'error' => $validatedFile['error']], $validatedFile['status']);
        }

        $file = $validatedFile['path'];
    ...
    private function validateImportFilePath($encodedFile)
    {
        $file = base64_decode($encodedFile, true);
        if ($file === false || $file === '' || strpos($file, "\0") !== false) {
            return ['ok' => false, 'status' => 400, 'error' => 'Invalid file parameter'];
        }

        // Normalize and reject traversal / absolute paths in the encoded relative path.
        $normalized = str_replace('\\', '/', $file);
        if ($normalized[0] === '/' || preg_match('#(^|/)\.\.(/|$)#', $normalized)) {
            return ['ok' => false, 'status' => 403, 'error' => 'Access to this file is not allowed'];
        }

        $allowedDirs = [];
        foreach (['imports', 'uploads'] as $dir) {
            $real = realpath(storage_path('app/'.$dir));
            if ($real) {
                $allowedDirs[] = $real;
            }
        }
        ...
        foreach ($allowedDirs as $allowedDir) {
            ...
            if ($realPath === $allowedDir || strpos($realPath, $allowedPrefix) === 0) {
                $insideAllowed = true;
                break;
            }
        }
        ...
        return ['ok' => true, 'status' => 200, 'error' => null, 'path' => $realPath];
    }

    /**
     * Detect MIME type from file content via fileinfo (not client-supplied headers).
     */
    private function detectImportFileMime($file)
    {
        ...
        // Fallback when fileinfo is unavailable.
    ...
    /**
     * Extension-scoped MIME allowlist. application/zip is only accepted for .xlsx
     * because OOXML packages are ZIP containers and fileinfo often reports that.
     */
    private function isAllowedImportMime($ext, $mime)
    {
        if (! is_string($mime) || $mime === '') {
            return false;
        }
    ...
    /**
     * Content/magic-byte checks so a renamed script cannot pass as a spreadsheet.
     */
    private function isSafeImportFileContent($file, $ext)
    {
        $path = method_exists($file, 'getRealPath') ? $file->getRealPath() : null;
        if (! $path || ! is_readable($path)) {
            return false;
        ...
        if ($ext === 'xlsx') {
            // ZIP local-file header; then require OOXML [Content_Types].xml.
        ...
        if ($ext === 'xls') {
            // OLE Compound Document signature (BIFF .xls).
    ...
    public function postDoUploadImportData()
    {
        ...
        if (! config('crudbooster.IMPORT_UPLOAD_HARDENING_ENABLED', true)) {
            return $this->postDoUploadImportDataLegacy();
        }

        $file = Request::file('userfile');
        $maxKb = (int) config('crudbooster.DEFEAULT_UPLOAD_MAX_SIZE', 1000);
        if ($maxKb < 1) {
            $maxKb = 1000;
        }
        ...
        if (! in_array($ext, ['xls', 'xlsx', 'csv'], true)) {
            return redirect()->back()->with([
                'message' => 'The userfile must be a file of type: xls, xlsx, csv.',
                'message_type' => 'warning',
            ]);
        }

        $detectedMime = $this->detectImportFileMime($file);
        if (! $this->isAllowedImportMime($ext, $detectedMime) || ! $this->isSafeImportFileContent($file, $ext)) {
            return redirect()->back()->with([
                'message' => 'Invalid import file content. Only valid xls, xlsx, or csv files are allowed.',
                'message_type' => 'warning',
            ]);
        }

        // Ensure private import root exists so realpath() allowlist checks work.
        $importsRoot = storage_path('app/imports');
        if (! is_dir($importsRoot)) {
            @mkdir($importsRoot, 0755, true);
        }

        // Store under storage/app/imports (not public /uploads preview route).
        $filePath = 'imports/'.CB::myId().'/'.date('Y-m');
    ...
    /**
     * Legacy CRUDBooster behavior — only used when CB_IMPORT_UPLOAD_HARDENING_ENABLED=false.
     */
    private function postDoUploadImportDataLegacy()
    {
        $file = Request::file('userfile');
